...
class ModelCode extends CCodeModel
{
	...
	public $ci_base_model = 'MY_Model';
	public $ci_model_ext = '_model';
	...
	public function attributeLabels()
	{
		return array_merge(parent::attributeLabels(), array(
			...
			'softDelete'=>'Soft Delete Feature',
			'softDeleteField' => 'Soft Delete Column Name'
		));
	}
	...
	public function generateRules($table)
	{
		...
		foreach($table->columns as $column)
		{
			...
			if($r)
			{
				$rules[$column->name]['rules'][] = 'required';
			}
			if($column->type==='integer')
				$rules[$column->name]['rules'][] = 'integer';
				//$integers[]=$column->name;
			elseif($column->type==='double')
				$rules[$column->name]['rules'][] = 'numeric';
				//$numerical[]=$column->name;
			elseif($column->type==='string' && $column->size>0)
				$rules[$column->name]['rules'][] = "max_length[".$column->size."]";
		...
		foreach(Yii::app()->{$this->connectionId}->schema->getTables($schemaName) as $table)
		{
			...
			else
			{
				...
				foreach ($table->foreignKeys as $fkName => $fkEntry)
				{
					...
					$relations[$className]['_belongs_to']="array('".$relationName."' => array('model' => '$refClassName', 'primary_key' => '$fkName') );";
					//public $belongs_to = array( 'author' => array( 'model' => 'author_m' ) );
					//public $belongs_to = array( 'author' => array( 'primary_key' => 'post_author_id' ) );
					//'blog' => array('blog', 'Blog', 'blog_id'),
	...
	protected function generateClassName($tableName)
	{
		...
		return $className."".$this->ci_model_ext;
	}
